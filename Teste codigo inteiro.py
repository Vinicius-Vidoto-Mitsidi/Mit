...
Dimensoes_local = [36,75,36,75,300]
...
Dimensoes_vidro = [9.6,19.2,19.2,28.8,0]
...
#----------------------------------------------------------------------
# Aqui, importaremos a biblioteca "datetime" que nos permite lidar com tempo mais facilmente
import datetime
...
Hora_Fim = datetime.datetime.strptime("18:00", "%H:%M").time()
...
#ver = 'Y'
Lampadas =[[60,15,10],[100,5,24],[15,10,10]]
#while ver == 'Y' or ver == 'y':
    # nova entrada onde a estrutura é [potencai em watts, numero de lampadas desta potencia]
...
#ver = 'Y'
Equipamentos = [[600,5,24],[1200,1,5]]
#while ver == 'Y' or ver == 'y':
    # nova entrada onde a estrutura é [potencia em watts, numero de equipamentos desta potencia]
...
for s in range(1, len(Q_conducao_direta_list)):
    Q_conducao_direta = (Q_conducao_direta_list[s] + Q_conducao_direta_list[s - 1]) * 1 / 2  # em Watts

#-------------------------------------------------------------

#------------ Calculo de Calor por radiação em superficies envidraçadas -----------
...
print('kg de ar que entra por segundo {:.3f}'.format(ar_renova))
...
for x in range(len(Lampadas)): Calor_lampadas.append(0.25*Lampadas[x][0] * Lampadas[x][1] * (Lampadas[x][2] / 24))
Q_iluminacao = sum(Calor_lampadas)
...
for x in range(len(Equipamentos)): Calor_equipamentos.append(0.35*Equipamentos[x][0] * Equipamentos[x][1] * (Equipamentos[x][2] / 24))
Q_Equipamentos = sum(Calor_equipamentos)
...
Num_ar_BTU = []
for d in range(len(Lista_BTUs_ar)) : Num_ar_BTU.append(math.ceil(Q_total/(0.293071*Lista_BTUs_ar[d]))) #0,2929 é o fator de conversao entre BTU/h e watt;
...
print("\n[{} ocupam o predio com a primeira entrando as {} e a ultima saida as {}, ficando um total de {} Horas]".format(Num_pessoas, Hora_inicio, Hora_Fim, Delta_tempo,))
...
for n in range(len(Num_ar_BTU)): print("              {} ar-condicionados de {} BTUs/h, exigindo uma podencia de {:.3f} kW pelos seus compressores (não inverter); ".format(Num_ar_BTU[n],Lista_BTUs_ar[n],Num_ar_BTU[n]*Lista_BTUs_ar[n]*0.2929/(1000*3.49)))
print('        Ou')
for n in range(len(Num_ar_BTU)): print("              {} ar-condicionados de {} BTUs/h, exigindo uma podencia de {:.3f} kW pelos seus compressores (inverter); ".format(Num_ar_BTU[n],Lista_BTUs_ar[n],Num_ar_BTU[n]*Lista_BTUs_ar[n]*0.2929/(1000*4.89)))
print('        Ou')
...
pyplot.bar(['CD','ST','SO','AE','I','E','P'],[Q_conducao_direta, Q_radiacao_supTransparente, Q_radiacao_supOpaca, Q_ar_entra, Q_iluminacao,Q_Equipamentos, Q_pessoa])
pyplot.show()
pyplot.plot([1,2,3],Lista_temperatura)
pyplot.show()